...
<#
.SYNOPSIS
    Markdown Converter Skill Installer for Windows.

.DESCRIPTION
    Installs the markdown-converter skill to system or project agent skill directories.
...
function Test-Deps {
    ...
    try {
        ...
        Write-Success "[OK] uv available"
    }
    catch {
        Write-Warning "[WARN] uv not found. markitdown requires uv."
...
function Main {
    if (-not $System -and -not $Project) {
        ...
        Write-Host "  .\install.ps1 -System -Agent trae"
    ...
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  uvx markitdown input.pdf -o output.md"
    Write-Host "  uvx markitdown input.docx > output.md"
}
