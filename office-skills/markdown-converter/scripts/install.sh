#!/usr/bin/env bash
# Markdown Converter Skill Installer (macOS / Linux)
...
check_deps() {
    ...
    if ! command -v uvx &>/dev/null && ! command -v uv &>/dev/null; then
        echo -e "${YELLOW}[WARN]${NC} uv/uvx not found. markitdown requires uv."
...
main() {
    ...
    echo ""
    echo "Usage:"
    echo "  uvx markitdown input.pdf -o output.md"
    echo "  uvx markitdown input.docx > output.md"
}
