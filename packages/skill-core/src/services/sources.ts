...
function getWellKnownLocation(value: string): WellKnownLocation | null {
  ...
  const hostSegment = normalizedValue.split("/")[0];
  const host = (hostSegment ?? "").replace(/\/$/, "").toLowerCase();
  if (!isDomainHost(host)) {
    return null;
  }
...
export function parseGitSource(value: string): ParsedGitSource {
  ...
  if (githubRepo) {
    const owner = githubRepo[1];
    const repoName = githubRepo[2]?.replace(/\.git$/, "");
    if (owner && repoName) {
      return {
  ...
  if (gitlabRepo) {
    const owner = gitlabRepo[1];
    const repoName = gitlabRepo[2]?.replace(/\.git$/, "");
    if (owner && repoName) {
      return {
        url: `https://gitlab.com/${owner}/${repoName}.git`,
      };
    }
  }
