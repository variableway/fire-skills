...
function Install-Hooks {
    ...
    $gitHooksDir = (& git rev-parse --git-path hooks 2>$null)
    if (-not $gitHooksDir) {
...
4. **FINISH** -- Update the Issue body with the final record and close the Issue:
   ``````bash
   python $skillRelPath/scripts/orchestrate.py finish --agent-expansion "<scope/questions/acceptance>" --plan "<plan>" --execution "<execution notes/tests>" --message "<completion summary>"
   ``````
