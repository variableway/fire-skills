...
copy_to_canonical() {
    ...
    for entry in "$source"/*; do
        local b
        b="$(basename "$entry")"
        case "$b" in .git|node_modules|__pycache__|.DS_Store|.env|.env.local) continue ;; esac
        cp -R "$entry" "$target/"
    done
    for entry in "$source"/.*; do
        [[ -e "$entry" ]] || continue
        local b
        b="$(basename "$entry")"
        case "$b" in .|..|.git|.env|.env.local|.DS_Store) continue ;; esac
...
" 2>/dev/null || echo "skills/devops/git-workflow")
...
2. **PLAN** — Expand the task into assumptions, acceptance criteria, and a short implementation plan.

3. **IMPLEMENT** — Execute the task (code changes, tests, etc.)

4. **FINISH** — Update the Issue body with the final record and close the Issue:
   ```bash
   python '"$skill_rel_path"'/scripts/orchestrate.py finish --agent-expansion "<scope/questions/acceptance>" --plan "<plan>" --execution "<execution notes/tests>" --message "<completion summary>"
   ```

This applies to all tasks referenced from task files (e.g., `tasks/issues/dev-workflow.md`).

**Do NOT skip the workflow** — always create an Issue first, keep the Issue body as the canonical record, then close it after completion.
'
